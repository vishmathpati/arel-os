...
/**
 * PropertyIconPicker (Ch8 v4) — a Notion-style icon picker for property headers.
 * Two tabs: **Emoji** (the full @emoji-mart picker — same source as the page
 * editor) and **Icons** (the complete Lucide set, searchable, and COLORABLE via
 * the tag palette). Stores the icon as an emoji char or `"lucide:Name"`, plus an
 * optional `icon_color`. `ColumnIconView` renders a stored icon anywhere.
 */
...
/** Popular icons shown before searching (kept short for a fast first paint). */
const POPULAR = [
  "Star",
  "Heart",
  "Flag",
...
/** Render a stored icon (emoji or "lucide:Name") with an optional tag color. */
export function ColumnIconView({
  icon,
  color,
  className,
}: {
  ...
  if (icon.startsWith("lucide:")) {
    const Ico = LUCIDE[icon.slice(7)];
    if (!Ico) return null;
    return <Ico className={className} style={iconColorStyle(color)} />;
  }
  return (
...
export function PropertyIconPicker({
  ...
  value?: string;
  color?: string;
  /** Shown in the trigger when no custom icon is set (the type icon). */
  fallback: ReactNode;
  onChange: (icon: string | undefined, color: string | undefined) => void;
}) {
  ...
  return (
    ...
    >
      <PopoverTrigger asChild>
        <button
          type="button"
          className="flex size-5 shrink-0 items-center justify-center rounded hover:bg-hover"
          aria-label="Property icon"
        >
          {value ? <ColumnIconView icon={value} color={color} className="size-3.5" /> : fallback}
        </button>
      </PopoverTrigger>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-1">
              {PICKER_COLORS.map((c) => (
                <button
                  key={c.name}
                  type="button"
                  onClick={() => isLucide && onChange(value, c.name)}
                  title={c.label}
                  className={cn(
